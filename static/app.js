...
const trendChart = new Chart(chartCtx, {
    ...
    options: {
        scales: {
            y: { min: 0, max: 10, ticks: { color: '#aaa' } },
...
function toggleModal() {
    document.getElementById('log-modal').classList.toggle('visible');
}

function logManualEvent(message) {
    fetch('/log_manual_event', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: message })
    }).then(() => {
        updateDashboard(); // 
        if(document.getElementById('log-modal').classList.contains('visible')) {
            toggleModal(); // 
        }
    });
}
